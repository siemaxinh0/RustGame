...
// Wynik ostatniej rozgrywki - decyduje czy pokazujemy "PRZEGRANA" czy "WYGRANA".
...
impl Plugin for StatePlugin {
    fn build(&self, app: &mut App) {
        app.init_state::<GameState>()
            ...
            .add_systems(
                Update,
                play_again_button.run_if(in_state(GameState::GameOver)),
            );
    }
}
...
fn spawn_game_over_ui(mut commands: Commands, result: Res<GameResult>) {
    ...
    let (title, title_color, accent, subtitle) = if result.won {
        (
            "WYGRANA!",
